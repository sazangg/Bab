...
class GatewayRequestResolution(Protocol):
    org_id: UUID
    team_id: UUID
    project_id: UUID
    virtual_key_id: UUID
...
async def attach_gateway_request_resolution(
    *,
    gateway_request_id: UUID | None,
    resolved: GatewayRequestResolution,
    db: AsyncSession,
) -> None:
    if gateway_request_id is None:
        return
...
async def list_gateway_requests(
    *,
    ...
    policy_labels = await policy_read_models.get_policy_labels(
        org_id=org_id,
        policy_ids={
            request.final_access_policy_id
            for request in page_requests
            if request.final_access_policy_id is not None
        },
        db=db,
    )
    items = [
...
async def get_gateway_request_trace(
    *,
    ...
    events = await guardrail_read_models.list_gateway_request_events(
        org_id=org_id,
        gateway_request_id=gateway_request_id,
        db=db,
    )
    usage_records = await usage_facade.list_usage_records_for_gateway_request(
        gateway_request_id=gateway_request_id,
        org_id=org_id,
        db=db,
    )
    ...
    guardrail_event_traces = [
        GuardrailEventTrace(
            ...
            metadata=redact_trace_value(event.metadata),
