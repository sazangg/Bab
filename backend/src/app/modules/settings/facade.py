...
async def get_organization_settings(
    *,
    ...
    if settings is None:
        org = await workspace_facade.get_organization_identity(org_id=scope.org_id, db=db)
...
async def update_organization_settings(
    *,
    ...
    async with transaction(db):
        settings = await repository.get_settings(org_id=scope.org_id, db=db)
        if settings is None:
            org = await workspace_facade.get_organization_identity(
                org_id=scope.org_id,
                db=db,
            )
            settings = await repository.create_settings(
        ...
        if "organization_name" in payload.model_fields_set and payload.organization_name:
            await workspace_facade.update_organization_name(
                org_id=scope.org_id,
