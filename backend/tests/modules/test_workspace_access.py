...
async def test_validate_assignment_scope_normalizes_known_targets(
    ...
    org_scope = await service.validate_assignment_scope(
        organization_id=org.id,
        scope_type="org",
        db=db_session,
    )
    team_scope = await service.validate_assignment_scope(
        organization_id=org.id,
        scope_type="team",
        team_id=team.id,
        db=db_session,
    )
    project_scope = await service.validate_assignment_scope(
        organization_id=org.id,
        scope_type="project",
        team_id=team.id,
        project_id=project.id,
    ...
    key_scope = await service.validate_assignment_scope(
        organization_id=org.id,
...
async def test_validate_assignment_scope_rejects_invalid_relationships(
    ...
    with pytest.raises(ScopeNotFoundError):
        await service.validate_assignment_scope(
            organization_id=org.id,
            scope_type="project",
            team_id=other_team.id,
            project_id=project.id,
